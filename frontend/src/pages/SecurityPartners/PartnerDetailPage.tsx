...
export default function PartnerDetailPage() {
  ...
  const partner = useMemo(() => DUMMY_PARTNERS.find((p) => p.id === partnerId) ?? null, [partnerId]);

  const [bodyguards, setBodyguards] = useState<Bodyguard[]>(() =>
    DUMMY_BODYGUARDS.filter((b) => b.partnerId === partnerId)
  );
  const [bodyguardModal, setBodyguardModal] = useState<{ open: boolean; bodyguard: Bodyguard | null; readOnly?: boolean }>({
    open: false,
    bodyguard: null,
    readOnly: false,
  });
  const [bodyguardToDelete, setBodyguardToDelete] = useState<Bodyguard | null>(null);

  const handleSaveBodyguard = (bodyguardId: string | null, values: BodyguardFormValues) => {
    if (bodyguardId) {
      setBodyguards((prev) =>
      ...
      setBodyguards((prev) => [...prev, { id: newId, partnerId: partnerId!, ...values }]);
    }
  };
  ...
  if (!partnerId || !partner) {
    return (
      <Box sx={{ p: 3 }}>
        <Typography variant="body1" color="text.secondary">Partner not found.</Typography>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate("/security-partners")} sx={{ mt: 2 }}>
          Back to partners
        </Button>
      </Box>
    );
      ...
      <Container maxWidth={false} sx={{ px: { xs: 1.5, sm: 2, md: 3 }, maxWidth: "100%" }}>
          ...
          <PageHeader
            ...
            action={
              <Button
                variant="contained"
                startIcon={<AddIcon />}
                onClick={() => setBodyguardModal({ open: true, bodyguard: null, readOnly: false })}
                sx={{
        ...
        <Box sx={{ mt: 2 }}>
          <BodyguardsTable
            ...
            onBodyguardView={(b) => setBodyguardModal({ open: true, bodyguard: b, readOnly: true })}
            onBodyguardEdit={(b) => setBodyguardModal({ open: true, bodyguard: b, readOnly: false })}
            onBodyguardDelete={setBodyguardToDelete}
          />
        </Box>
        <BodyguardModal
          open={bodyguardModal.open}
          onClose={() => setBodyguardModal((prev) => ({ ...prev, open: false }))}
